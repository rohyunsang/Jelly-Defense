...
public class TestSlime : MonoBehaviour
{
    ...
    void StopNavAgent()
    {/* >> 이대로는 "Stop" can only be called on an active agent that has been placed on a NavMesh.
UnityEngine.StackTraceUtility:ExtractStackTrace ()이런 에러가 생김
        if (navAgent != null)
        {
...
if (closestEnemy != null)
{
    ...
    // 가장 가까운 적을 향해 이동
    navAgent.SetDestination(closestEnemy.position); //# 가까운 적이 앞에 있으면 여기서 움직임을 멈춰야죠 
    animator.SetTrigger("Attack01"); //# 가면서 떄리는게 아니라 멈추고 떄림 
}
else
{
...
StopCoroutine("Attack");
    ...
    IEnumerator Attack() //데미지를 주는 타이밍과 멈춤을 위함
    {
        StopNavAgent(); //멈추고
        animator.SetTrigger("Attack01"); //공격
       // yield return new WaitForSeconds(1f); //프레임 대기
