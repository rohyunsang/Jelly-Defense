...
public class DemonKing : MonoBehaviour, IEnemy
{
    //컴포넌트들
    ...
    // Berserk skill
    private bool berserkSkillActivated = false;

    // blind icon skill 
    private float lastSkillActivationTime = 0f;
    private float skillActivationInterval = 20f; // 스킬을 활성화할 간격 (20sec)
    ...
    void Update()
    {
        ...
        if (CurrentHP / MaxHP <= 0.6f && !berserkSkillActivated)
        {
            BerserkSkill(); // 방어막 활성화 함수 호출
        }
    ...
    public void AllTargetAttack()
    {
        // SlimeSpawnManager.instance.slimeParent 의 모든 자식오브젝트에 접근. 
        foreach(Transform slime in SlimeSpawnManager.instance.slimeParent.transform)
        {
            slime.GetComponent<ISlime>().GetHit(AttackDamage);
            GameObject instanceHeatEffect = Instantiate(heatEffect, slime);
            Destroy(instanceHeatEffect, 0.5f);
        } 
    }
    ...
    public void DemonKingSkill() // Blind ICon 
    {
        anim.SetTrigger("Attack01");
        UIManager.instance.UnvisibleSlimeSpawnIcon(); // 7 sec later call back => VisibleSlimeSpawnIcon()  
        UIManager.instance.ShuffleSlimeIcon();
        StopNavAgent();
        StartCoroutine(ResumeMovementAfterAttack());
    }

    public void BerserkSkill()
    {
        berserkSkillActivated = true;

        Instantiate(buffPrefab, transform.position, Quaternion.identity, transform); // 현재 위치에 버프 생성
        // SlimeSpawnManager.instance.slimeParent 의 모든 자식오브젝트에 접근. 
        foreach (Transform slime in SlimeSpawnManager.instance.slimeParent.transform)
        {
            slime.GetComponent<ISlime>().GetHit(500f);
            GameObject instanceHeatEffect = Instantiate(heatEffect, slime);
            Destroy(instanceHeatEffect, 0.5f);
        }
    }
